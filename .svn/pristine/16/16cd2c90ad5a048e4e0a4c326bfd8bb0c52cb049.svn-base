...
public class Histogram {
	...
	public static float[][] makeHistogram(Image source, boolean hsbHistogram) {
		...
		// Begin Student Code
		int numBuckets = Math.min(width, 256);
		for (int x=0; x < width; x++) {
			for (int y=0; y < height; y++) {
				int pix = source.getPixel(x,y);
				int[] pixelChannelValues = {Image.pixelRed(pix), Image.pixelGreen(pix), source.pixelBlue(pix)};
				if (hsbHistogram) {
					// Because java ...
					int r = (int)pixelChannelValues[0] & 0xFF;
					int g = (int)pixelChannelValues[1] & 0xFF;
					int b = (int)pixelChannelValues[2] & 0xFF;
					float[] hsbChannelVals = Color.RGBtoHSB(r,g,b, null);
					for (int v=0; v < hsbChannelVals.length; v++) {
						float hsbChannelVal = hsbChannelVals[v];
						int bucketNum = Math.min((int) Math.round(hsbChannelVal * numBuckets), numBuckets-1);
						histogram[bucketNum][v] += 1;
					}
				} else {
					for (int c=0; c < pixelChannelValues.length; c++) {
						int pixelChannelValue = pixelChannelValues[c] &0xFF;
						int bucketNum = Math.min((int) Math.round(pixelChannelValue/scale), numBuckets-1);
						histogram[bucketNum][c] += 1;
					}
				}
			}
		}
